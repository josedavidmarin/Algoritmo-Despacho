...
def graficar_gantt(grafico_gantt):
    ...
    for i, (pid, inicio, fin) in enumerate(grafico_gantt):
        gantt.broken_barh([(inicio, fin - inicio)], (i + 0.5, 0.9), facecolors=('tab:blue'))
        gantt.text(inicio + (fin - inicio) / 2, i + 1, f"P{pid}", ha='center', va='center', color='white')
    plt.show()
